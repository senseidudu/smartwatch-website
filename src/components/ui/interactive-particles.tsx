...
import s from './interactive-particles.module.css'

/*
 * Vendored from the VengeanceUI registry:
 * https://raw.githubusercontent.com/Ashutoshx7/VengeanceUI/main/public/r/interactive-particles.json
 *
 * The WebGL effect is kept whole: an image is sampled into instanced particles that breathe on
 * simplex noise and scatter around the cursor through an off-screen touch texture, with the
 * GSAP intro that condenses them into place. Adapted for this site:
 *   - the upload control, its state and the object-URL bookkeeping are dropped; the footer
 *     passes a generated wordmark image as `src`
 *   - GSAP comes from the site's registered instance, and Tailwind classes become a CSS module
 *   - three.js is only ever reached through a lazy import, so it never lands in the main bundle
 */
...
export interface InteractiveParticlesProps {
  ...
  className?: string;
  /** Wrapper background color. Defaults to transparent. */
  background?: string;
...
export function InteractiveParticles({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // single WebGL context when the effect re-runs (e.g. after a prop change).
      renderer.dispose();
    };
  }, [effectiveSrc, color, size, randomness, depth, touchRadius, threshold, maxDimension]);

  return (
    <div ref={containerRef} className={className ? `${s.root} ${className}` : s.root} style={{ background }}>
